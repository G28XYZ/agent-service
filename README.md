...
12. В композере есть чекбокс `Chat mode + apply`: запрос идет как обычный чат, затем сервис пытается подготовить изменения в файлах по code-блокам ответа.
13. Корень workspace для desktop:
